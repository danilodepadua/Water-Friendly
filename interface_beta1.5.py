...
with con: 
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS Plantacoes (id INTEGER PRIMARY KEY,User TEXT, Planta_Nome TEXT UNIQUE, Tamanho INTEGER, Cultura TEXT, FOREIGN KEY(User) REFERENCES Usuarios(apelido) ON DELETE CASCADE)")
...
class WaterFriendlyApp:
    ...
    def Adicionar_Sensores(self, Quantidade, id):
        ...
        cursorr.execute("PRAGMA foreign_keys = ON") # Habilitar as chaves estrangeiras
        for i in range(0, Quantidade):
            cursorr.execute("INSERT INTO Sensores (id_plantacao, absorsao) VALUES (?, ?)", (id, random.randint(1, 10)))  # Inserir os dados na tabela
    ...
    def Renovar_Sensores(self, Quantidade, id):
        ...
        cursor.execute("PRAGMA foreign_keys = ON") # Habilitar as chaves estrangeiras
        cursor.execute("SELECT COUNT(*) FROM Sensores WHERE id_plantacao = ?", (id,)) # Consulta para obter a quantidade de sensores
        QuantidadeAtual = cursor.fetchone()[0]
        sensores = cursor.execute('SELECT * FROM Sensores WHERE id_plantacao = ?', (id,)).fetchall()
        if QuantidadeAtual < Quantidade:
            for i in range(0, Quantidade - QuantidadeAtual):
                cursor.execute("INSERT INTO Sensores (id_plantacao, absorsao) VALUES (?, ?)", (id, random.randint(1, 10)))
        elif QuantidadeAtual > Quantidade:
            for i in range(0, QuantidadeAtual - Quantidade):
                id_sensor = sensores[i][0]
                cursor.execute("DELETE FROM Sensores WHERE ident = ?", (id_sensor,))
    ...
    def adicionar_area(self):
        ...
        # Editando o frame--------------------------------------------
        self.Entry_Nome = customtkinter.CTkEntry(
            master=self.frame_adicionar_area, width=300, placeholder_text='Nome da plantação')
        self.Entry_Nome.place(x=350, y=200)
        self.entry_Sensores = customtkinter.CTkEntry(master=self.frame_adicionar_area, width=300, placeholder_text='Quantidade de sensores')
        self.entry_Sensores.place(x=25, y=300)
        self.ComboBox_Cultuta = customtkinter.CTkComboBox(master=self.frame_adicionar_area, width=300, values=list(PlantasInfos.keys()))
    ...
    def acessar_planta(self, planta_nome):
        ...
        cursor.execute("SELECT * FROM Plantacoes WHERE Planta_Nome = ?", (planta_nome,))
        resultado = cursor.fetchone()
        tamanho = resultado[3]
        id = resultado[0]
        print(AguaChuva(2, id, tamanho))
        con.close()  # Fechar a conexão com o banco de dados    
    ...
    def alterar_tamanho(self, planta_nome):
        ...
        self.entry_Sensores2 = customtkinter.CTkEntry(master=self.frame_alterar_tamanho, width=300, placeholder_text='Quantidade de sensores')
        ...
        def salvar_tamanho(planta_nome):
            ...
            id = cursor.execute("SELECT id FROM Plantacoes WHERE User = ? AND Planta = ?", (usuario_logado, planta_nome)).fetchone()[0]
    ...
    # criando a tela de perfil---------------------------------------------------
    def AguaChuva(self,MMChuva, id, areatotal):
        con = lite.connect('water.db')  # Abrir a conexão com o banco de dados
        cursor = con.cursor()  # Criar um cursor para executar as consultas
        cursor.execute("SELECT COUNT(*) FROM Sensores WHERE id_plantacao = ?", (id,)) # Consulta para obter a quantidade de sensores
        quant = cursor.fetchone()[0]
        cursor.execute("SELECT * FROM Sensores WHERE id_plantacao = ?", (id,)) # Consulta para obter a quantidade de sensores
        Sensores = cursor.fetchall()
        AguaTotal = 0
        div = int(areatotal)*10000/quant
        for i in range(0, quant):
            variacao = Sensores[i][2]*0.1
            AguaTotal += MMChuva*div*variacao
        con.close()  # Fechar a conexão com o banco de dados
        return AguaTotal
    ...
    def alterar_tamanho(self, planta_nome):
        ...
        botao_salvar = customtkinter.CTkButton(
            ...
            font=('Josefin Sans bold', 14), fg_color=laranja,
            command=lambda: salvar_tamanho(planta_nome))
        botao_salvar.place(x=400, y=450)
        # Botão de voltar--------------------------------------------
    ...
    def adicionar_imagem(self):
        ...
        if filename:
            ...
            temp_filename = os.path.join(pasta_destino, f"{usuario_logado}_temp_rounded_image.png")
    ...
    def alterar_cidade(self):
        ...
        def salvar_cidade():
            ...
            if not nova_cidade and not novo_pais:
                ...
                cursor = con.cursor()
                cursor.execute("PRAGMA foreign_keys = ON")
                cursor.execute("UPDATE Usuarios SET cidade = ?, pais = ? WHERE apelido = ?", (
